...
use cosmwasm_std::{entry_point, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult, Uint128, Decimal, to_binary};
...
pub fn execute_open_position(pair: String, side: u8, quote_asset_amount: Uint128, leverage: Decimal, base_asset_amount_limit: Uint128) -> Result<Response<NibiruMsg>, ContractError> {
    if pair.eq("") {
...
pub fn query(deps: Deps<NibiruQuery>, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Position { 
            trader, 
            pair 
        } => to_binary(&get_position(deps, trader, pair)),
        QueryMsg::Positions { 
            trader, 
        } => to_binary(&get_positions(deps, trader)),
    }
}

fn get_position(deps: Deps<NibiruQuery>, trader: String, pair: String) -> PositionResponse {
    let querier = NibiruQuerier::new(&deps.querier);
    querier.position(trader, pair).unwrap()
}

fn get_positions(deps: Deps<NibiruQuery>, trader: String) -> PositionsResponse {
    let querier = NibiruQuerier::new(&deps.querier);
