...
mod tests {
    ...
    pub fn mock_query_execute(
        ...
        match query_msg {
            QueryPerpMsg::AllMarkets {} => {
                to_binary(&dq::all_markets_response()).into()
            }
            QueryPerpMsg::BasePrice {
                pair: _,
                is_long: _,
                base_amount: _,
            } => to_binary(&dq::base_price_response()).into(),
            QueryPerpMsg::Position { trader: _, pair: _ } => {
                to_binary(&dq::position_response()).into()
            }
            QueryPerpMsg::Positions { trader: _ } => {
                to_binary(&dq::positions_response()).into()
            }
            QueryPerpMsg::Metrics { pair: _ } => {
                to_binary(&dq::metrics_response()).into()
            }
            QueryPerpMsg::ModuleAccounts {} => {
                to_binary(&dq::module_accounts_response()).into()
            }
            QueryPerpMsg::ModuleParams {} => {
                to_binary(&dq::module_params_response()).into()
            }
            QueryPerpMsg::PremiumFraction { pair: _ } => {
                to_binary(&dq::premium_fraction_response()).into()
            }
            QueryPerpMsg::Reserves { pair: _ } => {
                to_binary(&dq::reserves_response()).into()
            }
            QueryPerpMsg::OracleExchangeRate { pair: _ } => {
    ...
    fn oracle_exchange_rate_query() {
        ...
        // Call the query
        let req: QueryRequest<QueryPerpMsg> = QueryPerpMsg::OracleExchangeRate {
            pair: String::from("ETH:USD"),
        }
        .into();
        let querier_wrapper = QuerierWrapper::new(&deps.querier);
