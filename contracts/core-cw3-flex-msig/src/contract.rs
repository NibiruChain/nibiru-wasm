...
use cosmwasm_std::{
    to_binary, Binary, BlockInfo, CosmosMsg, Deps, DepsMut, Empty, Env,
...
mod tests {
    ...
    use cw_multi_test::{
        next_block, App, AppBuilder, BankSudo, Contract, ContractWrapper,
        Executor, SudoMsg,
    };
    ...
    fn test_instantiate_works() {
        ...
        let version =
            query_contract_info(&app.wrap(), flex_addr.clone()).unwrap();
        assert_eq!(
            ContractVersion {
    ...
    fn get_tally(app: &App, flex_addr: &str, proposal_id: u64) -> u64 {
        ...
        let votes: VoteListResponse =
            app.wrap().query_wasm_smart(flex_addr, &voters).unwrap();
    ...
    fn test_proposal_queries() {
        ...
        let info: Vec<_> =
            res.proposals.iter().map(|p| (p.id, p.status)).collect();
        ...
        let (expected_msgs, expected_title, expected_description) =
            proposal_info();
